...
//
// 어제까지 나간 과제만 센다. 오늘 과제는 세지 않는다 — 세면 매일 오후마다
// 반 전원이 미제출로 잡혀서, 정작 챙겨야 할 학생이 묻힌다.
// 어제 과제는 오늘까지 낼 수 있지만("월요일 과제는 화요일까지") 센다.
// 화요일에 월요일 과제를 안 낸 학생을 챙기는 게 교사가 원하는 것이다.
// 아직 낼 수 있는지는 화면이 칩 색으로 가른다.
//
...
export function pendingHomeworkStudents({ students = [], sets = [], days = [], submissions = [], today }) {
  ...
  // 어제까지의 요일만, 날짜순으로.
  // 열어준 요일도 남긴다 — 열어줬어도 아직 안 낸 건 안 낸 것이다.
